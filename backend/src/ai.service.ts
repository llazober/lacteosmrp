...
- En tu respuesta durante el tour, usa este formato:
  ...
  private async ejecutarNavegacion(user: any, seccion: string) {
    const PAGE_PERMISSIONS: Record<string, { permission: string; path: string }> = {
      ...
      planificacion_produccion: { permission: 'VER_PRODUCCION', path: '/produccion/planificacion' },
      ruta_operaciones: { permission: 'VER_PRODUCCION', path: '/produccion/operaciones' },
