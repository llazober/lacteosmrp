...
> [!IMPORTANT]
> **Run on the Host OS:** All commands in this guide (such as `docker`, `ufw`, `free`, and host-level utilities) must be run directly on the **host DigitalOcean droplet terminal (via SSH)**. Do not run them inside an application or database container shell (e.g., if your command prompt is prefixed with a container ID like `root@1b4a63897531:/#`, you are inside a container, and host commands like `docker` will fail with `command not found`).
...
> [!NOTE]
> All Docker commands below must be run on the **host system** (not inside any app or db containers).
